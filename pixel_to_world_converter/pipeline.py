...
def build_frame_homographies(
    ...
    """
    Precompute a 3x3 ground-plane homography for every video frame.

    Runs pipeline stages 1–4 (sensor alignment, Kalman filter, RTS smoother,
    camera extrinsics) then computes one homography per frame that maps pixel
    coordinates directly to ENU world coordinates at altitude ground_z.
    ...
    Pipeline Stages:
        ...
        3. Refine estimates using backward RTS smoother
        4. Compute camera extrinsic matrices from gimbal angles
        5. Compute per-frame ground-plane homographies
    ...
    Returns:
        ndarray of shape (video_num_frames, 3, 3) — one homography per frame.
        Apply with project_bbox() for ad-hoc bounding box projection.
    ...
    # ============================================================
    # SECTION 5: Compute per-frame ground-plane homography matrices
    # ============================================================
    pixel_size = (SENSOR_WIDTH / video_width, SENSOR_WIDTH / video_width)
    camera_intrinsic = compute_intrinsic_matrix(FOCAL_LENGTH, pixel_size, video_width, video_height)
    camera_intrinsic_inv = np.linalg.inv(camera_intrinsic)

    homographies = np.zeros((video_num_frames, 3, 3))
    for i in range(video_num_frames):
        homographies[i] = compute_ground_homography(
            camera_intrinsic, camera_extrinsics[i], ground_z, K_inv=camera_intrinsic_inv
        )
